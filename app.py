...
# ==================== PAGE: MAP (agences + datacenters) ====================
    ...
    if ag_f.empty and dc_f.empty:
        ...
        for op, color in [("Moov", OP_COLORS["Moov"]), ("Togocom", OP_COLORS["Togocom"])]:
            ...
            fig.add_trace(go.Scatter(x=d["lon"], y=d["lat"], mode="markers",
                                      marker=dict(size=8, color=color, line=dict(width=0.5, color="white")), name=op,
                                      text=d["nom"], hoverinfo="text"))
        fig.add_trace(go.Scatter(x=dc_f["lon"], y=dc_f["lat"], mode="markers",
...
# ==================== PAGE: MOBILE MONEY ====================
    ...
    with c1:
        ...
        if mm_f.empty:
            ...
            fig = px.pie(op_counts, names="operateur", values="count", hole=0.5,
                         color_discrete_sequence=[T["green"], T["yellow"], T["red"], T["accent"]])
    ...
    gap_txt = ""
    if len(agg_region) > 1:
        r = agg_region.dropna(subset=["agents_mm_pour_10k_hab"]).sort_values("agents_mm_pour_10k_hab")
        if len(r) > 1:
...
# ==================== PAGE: COVERAGE ====================
    ...
    c1, c2 = st.columns(2)
    kpi_card(c1, f"{n_zero_ag} / {n_units}", t("coverage_kpi_cantons", lang))
