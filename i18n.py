...
TR = {
    ...
    "coverage_disclaimer": {"fr": "⚠️ **Note méthodologique** : aucune donnée brute de couverture réseaux mobiles cellulaires (2G/3G/4G/5G) n'est disponible dans les données ouvertes présentes. Cet onglet construit un **indicateur proxy** de couverture, fondé sur la présence d'agents mobile money par canton — l'absence totale de point de service dans un canton est un signal de risque d'exclusion numérique, mais ne prouve pas l'absence de signal réseau mobile. La présence d'un agent mobile money dans un canton ne garantit pas une couveture réseau mobile cellulaire sur tout le canton.",
                            "en": "⚠️ **Methodological note**: no raw cellular mobile networks coverage data (2G/3G/4G/5G) is available in the current open data. This tab builds a **proxy indicator** of coverage, based on the physical presence of mobile money agents per canton — a canton with no service point at all is a signal of digital exclusion risk, but does not prove the absence of mobile network signal. The presence of an mobile money agent in a canton does not guarantee a cellular mobile network coverage over all the canton."},
    ...
    "coverage_dynamic_insight": {
        "fr": "**Analyse de la sélection actuelle** — à la granularité **{level}**, **{pct_covered}%** des {n_units} unités sélectionnées disposent d'au moins un agent mobile money, et **{n_zero_ag} unités sur {n_units} n'ont aucune agence Moov/Togocom**. Les zones non couvertes ou mal desservies par le réseau mobile se situent principalement dans les régions rurales reculées et les zones enclavées, loin du Grand Lomé et des grands axes routiers qui sont mieux couverts. Le Grand Lomé est la zone la mieux couverte et la plus performante pour la voix et l'internet mobile Togocom et Moov. L'Observatoire National du Numérique chiffre le pourcentage de la population couverte par au moins un **réseau mobile 4G à 83.40%** en 2025.",
        "en": "**Current selection analysis** — at **{level}** granularity, **{pct_covered}%** of the {n_units} selected units have at least one mobile money agent, and **{n_zero_ag} out of {n_units} units have no Moov/Togocom agency at all**. Areas with no coverage or poor mobile network service are mainly located in remote rural regions and landlocked areas, far from the Greater Lome and the major roads which have better coverage. Greater Lome is the best-covered and most efficient area for voice and mobile internet provided by Togocom and Moov. The National Digital Observatory estimates that the percentage of the population covered by at least one **4G mobile network stands at 83.40%** in 2025.",
    },
